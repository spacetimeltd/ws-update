...
# the input field order the required output format can be achieved
    ...
    rollout_data_hash.each_pair do |id, record|
        if @mphash[id]
            ...
            @mphash.each_pair do |x,y|
                if @rollout_ids.index x
                    puts x + " <====> " + rollout_data_hash[@xref[x]]['path']
                    path_to_parent(
                        rollout_data_hash[@xref[x]]['path'],
                        rollout_data_hash,
                        @mphash[x]
                        )
                    # so after that @mphash will only contain the extra ids
                    puts "the id #{x} must be manually updated in these locations:\n"
                    puts "#{@mphash[x]}\n"
                end
            end;
